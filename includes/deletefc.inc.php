<?php
	...
	if (!isset($_SESSION['username']) || $_SESSION['user_type'] != 1 || $_SERVER['REQUEST_METHOD'] != 'POST') {
		...
		if (isset($_POST['fc_id'])){
			...
			$sql = "DELETE FROM flashcard WHERE fc_id = ?;";
			$stmt = mysqli_stmt_init($conn);
			if (!mysqli_stmt_prepare($stmt, $sql)) {
				echo "ERROR";
			} else {
				mysqli_stmt_bind_param($stmt, "s", $fc_id);
				mysqli_stmt_execute($stmt);
				echo "Delete Success";
			}
			mysqli_stmt_close($stmt);
		} else {
			if (isset($_POST['selected_flashcard'])){
				$temp_id = mysqli_real_escape_string($conn, $_POST['selected_flashcard']);
				$emp_id = explode(',', $temp_id);				
				$sql2 = "DELETE FROM flashcard WHERE fc_id = ?;";
